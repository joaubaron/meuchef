...
// NORMALIZAÇÃO: converte registros antigos (nome/link) para formato atual (titulo/conteudo)
receitasParaImportar = receitasParaImportar.map(r => {
// Formato antigo: tem "nome" mas não tem "titulo"
if (!r.titulo && r.nome) {
if (r.link) {
// Receita com link externo — gera conteúdo mínimo com o link
return {
titulo: r.nome,
conteudo: `<strong>${r.nome}</strong><br>🔗 <a href="${r.link}" target="_blank">${r.link}</a>`,
categoria: (r.categoria || 'diversos').toLowerCase(),
data: r.data || new Date().toLocaleString()
};
} else if (r.conteudo) {
// Tem nome mas também tem conteudo
return { ...r, titulo: r.nome };
} else {
return null; // descarta registros sem conteúdo e sem link
}
}
// Normaliza categoria para minúsculas
if (r.categoria) {
r.categoria = r.categoria.toLowerCase();
}
return r;
}).filter(r => r !== null);
